...
class PublicFormController extends Controller
{
    ...
    public function submit(Request $request, $token)
    {
        ...
        try {
            $response = DB::transaction(function () use ($request, $formLink, $validated) {
                $formLink = FormLink::where('id', $formLink->id)->lockForUpdate()->firstOrFail();

                if (!$formLink->canAcceptSubmission()) {
                    return FormatResponseJson::error(false, 'Form sudah tidak dapat menerima submission.', 503);
                }

                $submissionUuid = $validated['submission_uuid'] ?? Str::uuid()->toString();

                // Check existing submission by UUID first
                $existingCompany = CompanyInformation::where('submission_uuid', $submissionUuid)->first();
                if ($existingCompany) {
                    Log::warning('Duplicate submission detected by submission_uuid for form_link_id: ' . $formLink->id . ' uuid: ' . $submissionUuid);
                    if ($request->wantsJson()) {
                        return FormatResponseJson::success(true, 'Data berhasil dikirim! Terima kasih.');
                    }
                    return redirect()->route('public.form.success');
                }
                ...
                Log::info('✅ Step 1 - Company created: ' . $company->id);
                ...
                Log::info('✅ Step 2 - Liable persons saved');
                // 3. Simpan Contacts
                ...
                Log::info('✅ Step 3 - Contacts saved');
                // 4. Simpan Addresses
                ...
                Log::info('✅ Step 4 - Addresses saved');
